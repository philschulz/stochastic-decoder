...
class TrainingModel(model.SockeyeModel):
    ...
    def _build_module(self, train_iter: data_io.ParallelBucketSentenceIter):
        ...
        init_labels = mx.sym.Variable(C.TARGET_LABEL_NAME)
        labels = mx.sym.reshape(data=init_labels, shape=(-1,))
        inv_temperature = None
        if self.config.annealing:
            inv_temperature = mx.sym.Variable(C.ANNEALING_NAME)
            expanded_inv_temperature = mx.sym.reshape(data=mx.sym.broadcast_mul(lhs=inv_temperature.expand_dims(axis=1), rhs=mx.sym.ones_like(init_labels)), shape=(-1,))

        model_loss = loss.get_loss(self.config.config_loss)
        if self.config.config_latent_encoder_sampler:
            global_kl_loss = loss.KLLoss(name="global_kl_loss",
                                         distribution_name=self.config.config_latent_encoder_sampler.distribution_name)
        if self.decoder.config.stochastic_rnn:
            position_wise_kl_loss = loss.KLLoss(name="position_wise_kl", distribution_name=C.DIAGONAL_GAUSS)
        ...
        def sym_gen(seq_lens):
            ...
            (label_embed,
             label_embed_length,
             label_embed_seq_len) = self.embedding_target.encode(init_labels, target_length, target_seq_len)
            ...
            latent_value = mx.sym.zeros(0)
            if self.config.config_latent_encoder_sampler:
                if self.config.latent_variable_average_states:
                    source_length_expanded = mx.sym.expand_dims(data=source_length, axis=1)
                    # not using mean here because some positions in the sequence are 0 pads
                    source_summary = mx.sym.sum(data=source_encoded, axis=0, keepdims=False)
                    source_summary = mx.sym.broadcast_div(lhs=source_summary, rhs=source_length_expanded)
                else:
                    source_summary = mx.sym.SequenceLast(data=source_encoded, sequence_length=source_encoded_length,
                                                         use_sequence_length=True)
                latent_value, model_params = self.latent_encoder_sampler.sample(source_summary)
            if self.config.config_inference_sampler:
                # Block gradient from infnet into target embeddings
                (target_encoded,
                 target_encoded_length,
                 target_encoded_seq_len) = self.inference_target_encoder.encode(mx.sym.BlockGrad(target_embed),
                                                                                target_embed_length,
                                                                                target_embed_seq_len)
                if self.config.latent_variable_average_states:
                    target_length_expanded = mx.sym.expand_dims(data=target_length, axis=1)
                    # not using mean here because some positions in the sequence are 0 pads
                    target_summary = mx.sym.sum(data=target_encoded, axis=0, keepdims=False)
                    target_summary = mx.sym.broadcast_div(lhs=target_summary, rhs=target_length_expanded)
                else:
                    target_summary = mx.sym.SequenceLast(data=target_encoded, sequence_length=target_encoded_length,
                                                         use_sequence_length=True)
                # prevent gradient flow into model representation from infnet
                latent_value, inf_params = self.inference_latent_sampler.sample(
                    mx.sym.concat(mx.sym.BlockGrad(source_summary),
                                  target_summary, dim=1, name="concat_source_target_summaries"))

            target_backward_encoding = None
            if self.config.config_decoder.variational_training:
                # Block gradient flow from backward target encoder into target embeddings
                (target_backward_encoding,
                 target_backward_encoding_length,
                 target_backward_encoding_seq_len) = self.backward_target_encoder.encode(mx.sym.BlockGrad(label_embed),
                                                                                         label_embed_length,
                                                                                         label_embed_seq_len)
                # target_backward_encoding: (backward_target_seq_len, batch_size, num_hidden)
                target_backward_encoding, *_ = mx.sym.SequenceReverse(target_backward_encoding,
                                                                      use_sequence_length=True,
                                                                      sequence_length=target_backward_encoding_length)
                # target_backward_encoding: (batch_size, backward_target_seq_len, num_hidden)
                target_backward_encoding = mx.sym.swapaxes(data=target_backward_encoding, dim1=0, dim2=1)

            # decoder
            # target_decoded: (batch-size, target_len, decoder_depth)
            (target_decoded,
             srnn_params,
             srnn_inf_params) = self.decoder.decode_sequence(source_encoded,
                                                                          source_encoded_length,
                                                                          source_encoded_seq_len, target_embed,
                                                                          target_embed_length, target_embed_seq_len,
                                                                          target_backward_encoding,
                                                                          latent_value)

            # target_decoded: (batch_size * target_seq_len, rnn_num_hidden)
            target_decoded = mx.sym.reshape(data=target_decoded, shape=(-3, -1))
            ...
            if self.inference_latent_sampler:
                # Use positive KL here because we want to minimise the negative ELBO
                probs += global_kl_loss.get_loss(params_q=inf_params, params_p=model_params,
                                                 annealing_factor=inv_temperature)
            if self.decoder.config.stochastic_rnn:
                probs += position_wise_kl_loss.get_loss(params_q=srnn_inf_params, params_p=srnn_params, labels=labels,
                                                        annealing_factor=expanded_inv_temperature)
    ...
    def fit(self,
            ...
            annealing_max: Optional[float] = 1.0):
        """
        ...
        :param annealing_factor: Initial annealing factor.
        :param annealing_increase: Additional increase for annealing factor after each update.
        :param annealing_max: Maximum value up until which the annealig factor will be increased.
        ...
        self._fit(train_iter, val_iter, output_folder,
                  ...
                  annealing_increase=annealing_increase,
                  annealing_max=annealing_max)
    ...
    def _fit(self,
        ...
        if self.config.config_inference_sampler:
            # TODO philip: this hard coding is ugly
            kl_terms = 2 if self.config.config_decoder.stochastic_rnn else 1
            metric_train = mx.metric.CompositeEvalMetric(
                [loss.CrossEntropyElboMetric(self.config.config_loss, annealing_factor=annealing_factor,
                                             annealing_increase=annealing_increase, kl_terms=kl_terms),
                 loss.KlMetric(annealing_factor=annealing_factor, annealing_increase=annealing_increase, kl_terms=kl_terms),
                 TrainingModel.create_eval_metric(C.PERPLEXITY)])
            metric_val = mx.metric.CompositeEvalMetric(
                [loss.CrossEntropyElboMetric(self.config.config_loss, annealing_factor=annealing_factor,
                                             annealing_increase=annealing_increase, kl_terms=kl_terms),
                 loss.KlMetric(annealing_factor=annealing_factor, annealing_increase=annealing_increase, kl_terms=kl_terms),
                 TrainingModel.create_eval_metric(C.PERPLEXITY)])
        ...
        if isinstance(optimizer, SockeyeOptimizer):
            ...
            elif self.config.config_inference_sampler:
                # Use NegElbo here because the optimisers minimise a loss
                metric_loss = loss.CrossEntropyNegElboMetric(self.config.config_loss)
            else:
        ...
        while True:
            ...
            if self.config.annealing:
                batch.data[2] *= annealing_factor
            annealing_factor += annealing_increase
            if annealing_factor > annealing_max:
                annealing_factor = annealing_max
                annealing_increase = 0.0
            ...
            if train_state.updates > 0 and train_state.updates % checkpoint_frequency == 0:
                ...
                toc = time.time()
                logger.info("Checkpoint [%d]\tUpdates=%d Epoch=%d Samples=%d Time-cost=%.3f Annealing-factor=%.5f",
                            train_state.checkpoint, train_state.updates, train_state.epoch,
                            train_state.samples, (toc - tic), annealing_factor)
                tic = time.time()
