...
class InferenceModel(model.SockeyeModel):
    ...
    :param deterministic_decoding: If false, stochastic models will sample their latent values from the prior.
    ...
    def __init__(self,
                 ...
                 deterministic_decoding: bool = True) -> None:
        ...
        # Ensure that the inference encoder is not used at test time
        config.config_inference_sampler = None
        config.config_decoder.variational_training = False
        ...
        self.deterministic_decoding = deterministic_decoding
    ...
    def _get_encoder_module(self) -> Tuple[mx.mod.BucketingModule, int]:
        ...
        def sym_gen(source_seq_len: int):
            ...
            # TODO philip: make this much smaller if no latent value is used
            latent_value = mx.sym.zeros(shape=(self.batch_size, self.decoder.latent_dim))
            if self.latent_encoder_sampler:
                ...
                latent_value, *_ = self.latent_encoder_sampler.sample(source_summary, deterministic=self.deterministic_decoding)

            # initial decoder states
            decoder_init_states = self.decoder.init_states(source_encoded_batch_major,
                                                           source_encoded_length,
...
def load_models(context: mx.context.Context,
                ...
                decoder_return_logit_inputs: bool = False,
                cache_output_layer_w_b: bool = False,
                deterministic_decoding: bool = True) -> Tuple[List[InferenceModel], Dict[str, int], Dict[str, int]]:
    """
