...
def add_model_parameters(params):
    ...
    # global latent variable params
    model_params.add_argument("--latent-distribution",
                              type=str,
                              default=C.DIAGONAL_GAUSS,
                              choices=[C.DIAGONAL_GAUSS, C.FULL_RANK_GAUSS],
                              help="The distribution of the latent variable. Choices: %s(choices)s. "
                                   "Default: %(default)s")
    model_params.add_argument("--latent-dim",
                              type=int_greater_or_equal(0),
                              default=1,
                              help="The dimension of the latent variable for latent variable models. "
                                   "Default: %(default)s")
    model_params.add_argument("--stochastic-encoder-average-states", action='store_true',
                              help="Average the states of the stochastic encoder. Otherwise the last state"
                                   "is used to infer the distribution of the latent variable.")
    model_params.add_argument("--latent-value-into-attention", action="store_true",
                              help="Feed the latent value into the attention mechanism and not the decoder.")
    model_params.add_argument("--latent-value-initial-state-only", action='store_true',
                              help="Only feeds the latent value into the first hidden state of the decoder.")
    model_params.add_argument("--use-variational-approximation", action="store_true",
                              help="Creates an inference network for the global latent variable. The network has "
                                   "the same structure as the model's stochastic encoder. It uses that structure "
                                   "twice, two encode both source and target sentence. Notice that the inference "
                                   "net is only used during training.")
    model_params.add_argument("--rnn-decoder-latent-value-gating", action="store_true",
                              help="Gates the deterministic hidden state against the latent value.")
    model_params.add_argument("--latent-value-into-output", action='store_true',
                              help="Make the stochastic decoder latent variables influence the output distribtuion "
                                   "directly. By default they only influence that distribution through the RNN states.")

    # latent boundaries on target side
    model_params.add_argument("--use-boundary-model", action="store_true",
                              help="Infers boundary points between words. These are then used to influence the "
                                   "dynamics of the RNN decoder.")

    # character encoding of target words
    model_params.add_argument("--use-target-char-embeddings", action="store_true",
                              help="Use embeddings of target characters and a character RNN to encode the previous "
                                   "target word in the decoder.")
    model_params.add_argument("--target-char-embed-size", type=int_greater_or_equal(1), default=10,
                              help="If --use-target-char-embed is selected, this flag specifies the size of the "
                                   "character embeddings. Default: %(default)s.")
    model_params.add_argument("--target-char-rnn-num-hidden", type=int_greater_or_equal(1), default=100,
                              help="If --use-target-char-embed is selected, this flag specifies the size of the "
                                   "hidden state of the character RNN used to encode the target words. "
                                   "Default: %(default)s.")

    # stochastic decoder RNN
    model_params.add_argument("--rnn-decoder-stochastic", action='store_true',
                              help="Turn the decoder RNN into a stochastic RNN with a Gaussian latent variable at "
                                   "each target position. Training is done by variational inference.")
    model_params.add_argument("--rnn-decoder-stochastic-num-hidden", type=int_greater_or_equal(1), default=0,
                              help="Size of the latent variables in the stochastic decoder RNN. Defaults to "
                                   "the value of --latent-dim.")
    model_params.add_argument("--rnn-decoder-stochastic-prev-word", action='store_true',
                              help="The stochastic decoder latent variables depend on the previous target token in "
                                   "addition to the previous latent variable and the previous RNN state.")

    # annealing
    model_params.add_argument("--kl-annealing-factor", type=float, default=1.0,
                              help="Initial annealing factor for the kl term in variational learning.")
    model_params.add_argument("--kl-annealing-increase", type=float, default=0.0,
                              help="Increase of the annealing factor after each update.")
    model_params.add_argument("--kl-annealing-max", type=float, default=1.0,
                              help="Maximum value up until which the kl annealing factor will be increased.")
...
def add_inference_args(params):
    ...
    decode_params.add_argument('--sample-translations',
                               action='store_true',
                               help="Samples translations from stochastic models by sampling their latent variables "
                                    "from the respective priors. Does not have any effect on deterministic models.")
