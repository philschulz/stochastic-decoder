...
def create_data_iters(args: argparse.Namespace,
    ...
    return data_io.get_training_data_iters(source=os.path.abspath(args.source),
                                           ...
                                           provide_annealing_data=args.use_variational_approximation)
...
def create_decoder_config(args: argparse.Namespace,  encoder_num_hidden: int) -> Config:
    ...
    if args.decoder == C.TRANSFORMER_TYPE:
        ...
        stochastic_num_hidden = args.latent_dim
        if args.rnn_decoder_stochastic_num_hidden > 0:
            stochastic_num_hidden = args.rnn_decoder_stochastic_num_hidden
        config_decoder = decoder.RecurrentDecoderConfig(
            ...
            latent_value_into_hidden_state=args.latent_dim > 1,
            latent_value_into_attention=args.latent_value_into_attention,
            latent_value_initial_state_only=args.latent_value_initial_state_only,
            variational_training=args.use_variational_approximation,
            stochastic_rnn=args.rnn_decoder_stochastic,
            stochastic_layer_size=stochastic_num_hidden,
            latent_value_into_output=args.latent_value_into_output,
            stochastic_rnn_prev_word_dependence=args.rnn_decoder_stochastic_prev_word,
            latent_value_gating=args.rnn_decoder_latent_value_gating)
...
def check_encoder_decoder_args(args) -> None:
    ...
    if args.rnn_decoder_stochastic:
        check_condition(args.optimized_metric, "When using stochastic models --optimized-metric needs to be 'elbo'.")
...
def create_model_config(args: argparse.Namespace,
    ...
    config_latent_encoder = None
    config_inference_sampler = None
    if args.latent_dim > 1:
        config_latent_encoder = rep.ReparametrisationSamplerConfig(distribution_name=args.latent_distribution,
                                                                   latent_dim=args.latent_dim)

        if args.use_variational_approximation:
            config_inference_sampler = rep.ReparametrisationSamplerConfig(distribution_name=args.latent_distribution,
                                                                          latent_dim=args.latent_dim)
    ...
    model_config = model.ModelConfig(config_data=config_data,
                                     ...
                                     weight_normalization=args.weight_normalization,
                                     config_latent_encoder_sampler=config_latent_encoder,
                                     config_inference_sampler=config_inference_sampler,
                                     latent_variable_average_states=args.stochastic_encoder_average_states,
                                     annealing=args.use_variational_approximation)
    return model_config
...
def main():
    ...
    with ExitStack() as exit_stack:
        ...
        training_model.fit(train_iter, eval_iter,
                           ...
                           annealing_factor=args.kl_annealing_factor,
                           annealing_increase=args.kl_annealing_increase,
                           annealing_max=args.kl_annealing_max)
